...
* Refactor the ```create``` method to a switch or if block that evaluates the passed ```:language``` parameter and then calls ```translation.to{language}```. This is a needed step to make the translation service more extensible. I missed this in my refactoring and am now past the deadline to submit code, so I'll mention it here for future discussion. 
